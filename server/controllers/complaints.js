...
// console.log("Selected officer:", selectedOfficer);
  ...
  if (!selectedOfficer) {
    throw new NotFoundError("No officer available in this district");
  }
  ...
  await complaint.assignOfficer(selectedOfficer._id);
  console.log("Officers found:", officers);
console.log("Selected officer:", selectedOfficer);
...
const rateOfficer = async (req, res) => {
  try {
    console.log('rateOfficer called with:', req.params.id, req.body);
    ...
    console.log('Rating value:', numberofstars);
    
    const complaint = await Complaint.findOne({ _id: complaintId });
    console.log('Complaint found:', complaint ? complaint.subject : 'not found');
    ...
    const officer = await Officer.findOne({ _id: complaint.officerID });
    console.log('Officer found:', officer ? officer.name : 'not found');
    ...
    console.log('OfficerRating found:', officerRating ? 'yes' : 'no');

    if (!officerRating) {
      throw new BadRequestError("Officer rating record not found");
    }
    ...
    console.error('Error in rateOfficer:', error);
